...
const SearchPatients = () => {
    ...
    const fetchPatientHistory = async (patientId) => {
        setIsLoading(true);
        try {
            const response = await fetch(`http://localhost:8000/p/history/${patientId}`, {
                headers: {
                    "Accept": "application/json",
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${sessionStorage.getItem("jwt")}`,
    ...
    return (
        <div className="relative max-w-6xl mx-auto p-6 bg-white shadow-lg rounded-lg mt-10">
            ...
            <div className={`fixed top-0 right-0 h-full w-96 bg-white shadow-lg transform transition-transform duration-300 ease-in-out z-50 ${sidebarOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                <div className="p-6 h-full overflow-y-auto">
                    ...
                    {selectedPatient && (
                        <div className="mb-6 p-4 bg-gray-100 rounded-lg">
                            <h3 className="text-xl font-semibold text-gray-700 mb-2">{selectedPatient.name}</h3>
                            ...
                            <div className="space-y-4">
                                {patientHistory.map((item, index) => (
                                    <div key={index} className="border-l-4 border-green-600 pl-4 py-2">
                                        ...
                                        <h4 className="font-medium text-gray-800 mb-1">{item.type}</h4>
                                        <p className="text-gray-700">{item.description}</p>
                                        {item.treatment && (
                                            <p className="mt-1 text-green-700"><span className="font-medium">Treatment:</span> {item.treatment}</p>
                                        )}
                                    </div>
                                ))}
                            </div>
