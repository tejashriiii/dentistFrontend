...
const Prescriptions = () => {
  ...
  const updatePrescription = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/doc/prescription/${editingPrescription.id}/`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const deletePrescription = async (id) => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/doc/prescription/${id}/`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem("jwt")}`,
  ...
  const prescriptionTypes = [
    "All",
    ...new Set(prescriptions.map((item) => item.type)),
  ];
        ...
        <div className="bg-white p-4 rounded-lg shadow-md mb-6" ref={formRef}>
          ...
          <div className="flex flex-col gap-4 mt-2">
            ...
            <div className="flex gap-4">
              <button
                onClick={
                  editingPrescription ? updatePrescription : addPrescription
                }
                className="bg-[var(--darkgreen)] text-white px-4 py-2 rounded hover:bg-[var(--darkergreen)]"
              >
                {editingPrescription
