...
function About() {
  ...
  const getCurrentPatient = () => {
    ...
    return patientSelect == "No patients remaining" ? patientSelect.value : "";
  };
  ...
  useEffect(() => {
    ...
    if (patientSelect) {
      const handlePatientChange = () => {
        ...
        const savedPastIllnesses = sessionStorage.getItem(
          `pastIllnesses_${newPatient}`,
        );
        setPastIllnesses(
          savedPastIllnesses ? JSON.parse(savedPastIllnesses) : [],
        );

        const savedAllergies = sessionStorage.getItem(
          `allergies_${newPatient}`,
        );
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem(
      `pastIllnesses_${patient}`,
      JSON.stringify(pastIllnesses),
    );
  }, [pastIllnesses]);
        ...
        {[
          ...
          <div key={id} className="w-1/2">
            ...
            <div className="flex mt-2">
              ...
              <button
                onClick={() =>
                  handleAddItem(setter, document.getElementById(id)?.value, id)
                }
